...
namespace Game1.Player
{
    class PlayerStatePortalRight : IPlayerState
    {
        ...
        public PlayerStatePortalRight(IPlayer player, Vector2  position)
        {
            this.player = player;

            if (player.GetType() == typeof(Player1)) {
                Sprite = PlayerSpriteFactory.Instance.CreatePortalRightSprite();
            } else {
        ...
        public void UseItem()
        {
            ...
            {
                player.SpawnProjectile(new PortalProjectile(Util.CompassDirection.East, position, player, PortalGunManager.Instance.Player1Color));
